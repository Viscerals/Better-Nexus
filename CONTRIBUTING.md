...
4. Run `python tools/ci_check.py` (needs Python 3.9+ and `luajit`) and, for packaging or TOC changes, `python tools/build_package.py --check`. The README lists prerequisites. Every listed test runs from the checkout; a test reported as NOT RUN fails the check. A new test file must be added to `tools/run_prototype_tests.py`, or the inventory check fails.
