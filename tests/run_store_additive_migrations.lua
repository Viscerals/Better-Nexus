...
-- distinct legacy table is not destroyed or merged speculatively.
local current = {
    settingsVersion=2,
...
Store.Init()
assert(NexusDB == current and WishlistRealizerDB == staleLegacy
    and NexusDB.currentRoot and NexusDB.settings.currentOnly == "keep"
    and next(NexusDB.settings.anchorNames) == nil,
    "current and legacy SavedVariables collision clobbered one owner")
