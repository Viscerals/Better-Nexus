...
check(dialog.message:GetText():find('current permanent Echoes',1,true)
 and not dialog.message:GetText():find('owned locks',1,true),'T07/T12 actual role-picker explanation uses permanent-target vocabulary')
check(dialog.count:GetText():find('Rolled copies 79/79',1,true) and dialog.count:GetText():find('Permanent targets 6/6',1,true),'display-order-independent six matching suggestions')
...
check(equal(live,H.perks.serverBuildSlots) and equal(owned,H.locked),'server mirrors and permanent locks unchanged')
