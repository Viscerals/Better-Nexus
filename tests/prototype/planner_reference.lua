Nexus={}
dofile('logic/WishlistPilot.lua')
local ref={}
...
for case=1,10000 do
 ...
 local a,b=Nexus.WishlistPilot.Decide(input),ref.WishlistPlanner.Decide(input)
...
local out=Nexus.WishlistPilot.DecideNexus(state)
assert(out.type=='freeze' and out.index==1 and out.spellId==11)
assert(out.outstanding==2)
state.board.cards[1].isFrozen=true
out=Nexus.WishlistPilot.DecideNexus(state); assert(out.type=='banish' and out.index==2)
state.owned.synced=false; assert(Nexus.WishlistPilot.DecideNexus(state).type=='wait')
state.owned.synced=true;state.ordinaryBoardAllowed=false
assert(Nexus.WishlistPilot.DecideNexus(state).type=='wait')
