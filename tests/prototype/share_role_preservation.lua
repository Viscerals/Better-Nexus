...
for _,case in ipairs({{78,6},{79,6},{41,6},{41,0},{1,1}})do
 ...
 if case[2]>0 then assert(label:find(case[1]..' / 79 + '..case[2]..' / 6 locked',1,true),'source menu states both role counts: '..label)
  ...
  local counts=label:sub(label:find(case[1]..' / 79',1,true) or 1)
  assert(label:find(case[1]..' / 79',1,true) and not counts:find('locked',1,true)
   and not counts:find('/ 6',1,true),'no locked copies, no locked role in the label: '..label)end
