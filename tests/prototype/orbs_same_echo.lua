...
-- An exact-quality change supplies a real state delta for the same spell ID.
-- Equal-content same-quality completion is separately required to stay paused.
H.Offer({{spellId=source,quality=q+1},{spellId=410008,quality=1},{spellId=410001,quality=1}})
...
check(M.Status().pending and M.Status().spent==1,'a fresh table with unchanged content still cannot confirm a replacement')
H.Result(source,q+1)
check(not M.Status().pending and M.Status().spent==1,'same-ID different-quality result confirms only with observed selection and exact state delta')
