...
assert(A.SetFirstLoadoutWishlistIdentity('Local locked design',entries))
...
local s=M.Status();assert(s.progress.permanentMissing==1,'assigned target includes the saved locked design')
assert(not M.Start() and H.Count('orb-spend')==0,'automatic pool protects the only future locked copy')
...
assert(M.Status().state=='NO_SOURCES' and H.Count('orb-spend')==1,'post-result protection retains the required locked copy')
M.Stop()
...
print('PASS automatic source pool includes durable locked targets, exclusion and post-result protection')
