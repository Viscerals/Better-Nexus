...
local pct,txt=Nexus.LoadingStatus.Progress({state='pending',progressDone=1,progressTotal=4})
check(pct==25 and txt:find('This step',1,true),'measured percentage explicitly phase-local')
local pct2=Nexus.LoadingStatus.Progress({state='pending',progressDone=5,progressTotal=4})
...
check(s.progressDone==1 and s.progressTotal==4,'read-only current-owner progress retained')
check(NexusLoadingOpenWishlist:IsEnabled(),'local navigation unlocks after real local readiness')
check(f.bar:GetValue()==25 and f.progress:GetText():find('25%%'),'actual panel renders measured phase percent')
local before=calls
