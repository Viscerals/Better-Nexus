...
check(dialog.count:GetText():find('Permanent targets 0/6',1,true),'manual choice does not silently suggest roles')
NexusWishlistRolePlus1:Click()
check(dialog.count:GetText():find('Permanent targets 1/6',1,true),'real row button changes the draft')
...
check(T.Equal(H.locked,original),'no permanent-lock changes in complete workflow')
