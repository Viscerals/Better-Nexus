...
assert(has(a.entries,200080),'first saved locked target reaches Orb authority')
local b=save(2,200081,'Plan B')
assert(has(b.entries,200081),'second saved locked target reaches Orb authority')
...
assert(has(restored.entries,200080) and not has(restored.entries,200081),'assigned Plan A must retain its own exact locked design after saving Plan B')
assert(H.Count('orb-spend')==0,'Plan A only acquired locked-target copy must never enter automatic sacrifice')
print('PASS two saved plans preserve separate locked designs and source protection')
