...
-- more than rootMapKeys (2048) keys, or when the distinct typed keys across
-- overlay and bundled exceed rows (2048). The active overlay is the saved
...
-- 5. Disjoint union: refused while scanning bundled, on the distinct-slot counter.
Ready('disjoint 1500 + 548',Boot(Db(Map('ov-',1,1500)),Map('base-',1,548)))
Refused('disjoint 1500 + 549',Boot(Db(Map('ov-',1,1500)),Map('base-',1,549)),'bundled','distinct-slots','legacy')
...
-- 7. At run time the current code cannot grow the overlay past the limit: a
-- received record for a 2049th slot is refused by the same collection check
-- in the mutation path, and the saved overlay stays at 2048.
do
 ...
 check(last and last.mode=='mutation' and last.count==2049,'runtime: the refusal is recorded as a mutation refusal')
 F.Reload()
