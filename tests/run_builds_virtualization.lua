...
for index = 1, 1000 do
    ...
    NexusDB.communityBuilds[id] = {
        ...
        fingerprint=tostring(710000+index).."x1",
...
assert(initial.results == 20 and initial.active <= 5
    ...
    string.format("Community result/pool mismatch: results=%s active=%s created=%s first=%s",
        tostring(initial.results),tostring(initial.active),
        tostring(initial.created),tostring(initial.first)))
...
C.Select("virtual-0015")
local offscreen = C.VirtualStats()
assert(offscreen.selectedId == "virtual-0015" and not offscreen.selectedVisible
    and C.GetSelectedBuildForPanel().id == "virtual-0015",
