...
function Codec.DecodeEBH1(str, allowRoleSelection)
    ...
    for chunk in body:gmatch("[^,]+") do
        ...
        if locked then
            ...
            ordinarySeen[id] = true
            ordinaryTotal = ordinaryTotal + stacks
            if ordinaryTotal > 79 and allowRoleSelection ~= true then return nil end
        end
        entries[#entries + 1] = {
    ...
    if #entries == 0 or ordinaryTotal + lockedCount > 85 then return nil end
    -- This bounded draft is never action evidence. The editor must obtain an
    -- explicit role choice before using it as an ordinary <=79-copy Wishlist.
    return { entries = entries, class = class, name = name,
        needsRoleSelection=ordinaryTotal > 79 and true or nil }
end
