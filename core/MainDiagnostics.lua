...
local function LogText_AutoLock()
    ...
    if wl then
        ...
        out[#out + 1] = "  (no assigned Wishlist could be read; check it in the editor)"
    end
    out[#out + 1] = ""
...
local function NewAIExportCoroutine()
    return coroutine.create(function()
        local logs = Nexus.DiagnosticLogs
        coroutine.yield("Starting diagnostic snapshot; no gameplay mutation")
        local snapshot = logs and (logs.SnapshotCooperative or logs.Snapshot)
        local log = snapshot and snapshot("decision",coroutine.yield) or {}
        coroutine.yield("Collecting run audit history")
        local audits = snapshot and snapshot("runAudit",coroutine.yield) or {}
        coroutine.yield("Collecting UI history")
        local probes = snapshot and snapshot("uiProbe",coroutine.yield) or {}
        local errors = {}
        ...
        if type(pageProvider) == "function" then
            for _, pageKey in ipairs(pageKeys) do
                coroutine.yield("Collecting " .. pageKey .. " page")
...
local function LogText_AIExport()
    return "Press Prepare full diagnostic report. Large reports may take a moment. Select this page and copy each page in order; one page is not the complete report."
end

local function LogText_Mismatch()
    local log = Nexus.DiagnosticLogs and Nexus.DiagnosticLogs.Snapshot
        and Nexus.DiagnosticLogs.Snapshot("decision") or {}
    local out = { "MANUAL OVERRIDES -- choices where your action differed", "" }
    local nMis = 0
    local first = math.max(1, #log - 99)
    if first > 1 then out[#out + 1] = "(newest 100 choice records; a different manual action is not necessarily wrong)"; out[#out + 1] = "" end
    for i = first, #log do
        local e = DiagnosticTable(log[i])
        local users = DiagnosticTable(e.user)
        local proposal = DiagnosticTable(e.proposal)
...
local function LogText_Locked()
    ...
    if #overflow > 0 then
        out[#out + 1] = string.format(
            "Legacy ID-based capacity estimate: %d entries may exceed the rolled cap. Check exact roles and copies in the editor before changing anything:", #overflow)
...
local function LogText_State()
    ...
    if refusal then
        out[#out + 1] = string.format(
            "LAST AUTO-SAVE REFUSAL [%s] L%s slot %s: %s",
            tostring(refusal.t), tostring(refusal.level),
            tostring(refusal.incumbentSlot), Nexus.UserText and Nexus.UserText.Message(refusal.detail) or tostring(refusal.detail))
        out[#out + 1] = "  Details: " .. tostring(refusal.detail)
        out[#out + 1] = "  (\"coverage lost: X\" means a previously matched target is now missing."
        out[#out + 1] = "   \"no net gain\" means no additional target progress was confirmed."
        out[#out + 1] = "   Extra or different-quality copies may also prevent an automatic save."
        out[#out + 1] = "   The detailed code is retained above for support.)"
        out[#out + 1] = ""
    end
    if slots then
