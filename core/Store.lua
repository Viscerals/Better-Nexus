...
local function BootstrapSlice(C)
    ...
    if state == SS.AUTHORITY then
        local db = C.database
        -- Initialization is once per coordinator/database, not once per catalog
        -- slice. Repeating Evidence.Init discards detached preparation and scans
        -- the same evidence memberships thousands of times. Catalog.Init below
        -- still resumes and validates its exact source/owner token on every pump.
        if not C.authorityOwnersInitialized then
            ...
            if Nexus.BuildCatalog
                and type(Nexus.BuildCatalog.BeginBootstrapSealV1) == "function"
                and not OwnerCall(C, "BuildCatalog.BeginBootstrapSealV1",
                    Nexus.BuildCatalog.BeginBootstrapSealV1) then
                return
            end
            C.authorityOwnersInitialized = true
        end
        ...
        if AccountWritesAllowed(db) and not readOnly then
            ...
            if catalog and type(catalog.Status) == "function"
                ...
                if catalog.RebindRequired() then
                    ...
                    local source = type(target)=="table"
                        and (rawget(target,"authorityBundle") or target) or nil
                    if evidence and type(evidence.Init)=="function"
                        and type(target)=="table"
                        and (C.rebindEvidenceDb~=target
                            or C.rebindEvidenceSource~=source
                            or C.rebindEvidenceOwner~=evidence
                            or C.rebindEvidenceInit~=evidence.Init) then
                        if not OwnerCall(C,"LoadoutEvidence.Init",evidence.Init,target) then
                            return
                        end
                        C.rebindEvidenceDb,C.rebindEvidenceSource=target,source
                        C.rebindEvidenceOwner,C.rebindEvidenceInit=evidence,evidence.Init
                    end
                    -- AUTHORITY-COORDINATOR-DRIVE END
