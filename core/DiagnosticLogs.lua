...
function Logs.SnapshotCooperative(name, yieldProgress)
    local captured, captureError = Protected(nil, function()
        local _, history, _, _, state = NormalizeUnsafe(name)
        if state.readOnly then return nil, state.reason end
        local refs, extensions = {}, {}
        CopyExtensions(history, extensions)
        for index = state.head, state.tail do refs[#refs+1] = history[index] end
        return {refs=refs,extensions=extensions}
    end)
    if not captured then error(captureError or "diagnostic snapshot unavailable", 0) end
    local output, copyError = DefensiveCopy(captured.extensions)
    if not output then error(copyError or "diagnostic extensions unavailable", 0) end
    for i,record in ipairs(captured.refs) do
        local copy, why = DefensiveCopy(record)
        if copy == nil then error(why or "diagnostic record unavailable", 0) end
        output[i] = copy
        if type(yieldProgress)=="function" and i%2==0 then
            yieldProgress("Collecting "..tostring(name).." "..i.."/"..#captured.refs)
        end
    end
    return output
end
