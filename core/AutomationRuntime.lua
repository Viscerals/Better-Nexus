...
local function AutoAllowed()
    ...
    if type(Adapter.OrdinaryBoardAllowed) == "function" then
        local allowed, why = Adapter.OrdinaryBoardAllowed()
        if not allowed then return false, why or "Orb state active or unknown" end
    end
    return true
end
...
local function SaveChangeSummary(owned, incumbentEchoes, plan, catalog, diag)
    ...
    local gained, shed = {}, {}
    local beforeProgress, afterProgress, gainedCopies, fewerCopies = 0, 0, 0, 0
    ...
    for fam in pairs(wished) do
        ...
        if d > 0 then
            gainedCopies=gainedCopies+d
            gained[#gained + 1] = name .. (d > 1 and (" x" .. d) or "")
        elseif d < 0 then
            local n = -d
            fewerCopies=fewerCopies+n
    ...
    local lead = "Matching Wishlist copies: +"..gainedCopies.." / -"..fewerCopies.." compared with the previous saved build"
    local out
    if #gained > 0 and #shed > 0 then
        out = lead .. " — gained " .. table.concat(gained, ", ")
            .. "; fewer requested copies of " .. table.concat(shed, ", ")
    elseif #gained > 0 then
        out = lead .. " — gained " .. table.concat(gained, ", ")
    elseif #shed > 0 then
        out = lead .. " — fewer requested copies of " .. table.concat(shed, ", ")
    else
        out = lead
    end
    ...
    if cleared > 0 then
        out = out .. "; reduced " .. cleared .. " wrong-quality cop"
...
local function PreauthorizeActionIntent(intent, settings)
    ...
    if (action.type == "freeze" and settings.autoFreeze == false)
        or (action.type == "reroll" and settings.autoReroll == false)
        or (action.type == "banish" and settings.autoBanish == false) then
        FinishActionIntent("superseded", "action_setting_disabled", false)
        return false, "action_setting_disabled"
    end
...
local function StepRun(level, plan, slots, owned, flags, disabledLevers, static,
    ...
    local state = {
        ...
        searchRefused = {
            ...
            reroll = settings.autoReroll == false or refusedRerollSig == board.signature,
        },
        rerollBudget = {
...
local function JournalData()
    ...
    if not wishlist then
        ...
        local lines = {
            string.format("Wishlist '%s'%s: %d Echo families -- %d represented, %d missing, %d off-Wishlist families in the saved build",
        ...
        for _, f in ipairs(filler) do
            fillerLines[#fillerLines + 1] = f .. (flags.DISABLE_SUPPRESSES_GUARANTEE
                and "  (not on the target plan; compare after the next run)" or "  (a later run may reduce these extras)")
        end
        sections[#sections + 1] = { title = "Extra copies compared with the plan", lines = fillerLines }
    end
...
local function ClearStepRetry()
    ...
    function M.ToggleAuto()
        if not autoEnabled and Nexus.OrbRuntime and Nexus.OrbRuntime.BlocksOrdinary() then
            SetStatus("Ordinary Automation stays OFF while the Orb operation is active or unresolved.")
            return false
        end
