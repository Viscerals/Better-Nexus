...
4. In My Builds, select the intended Saved Build and assign the Wishlist to it. Assigning only chooses the target; it does not change the Saved Build.
5. Review recommendations with Auto OFF first. With Auto ON, Nexus may later replace that active Saved Build after a better completed run: it treats the slot as a working copy, not a protected archive. Keep Auto OFF if you want it left unchanged.
...
Automatic save is separate from Take, Banish, Reroll and Freeze. After a completed run, Auto may replace your active Saved Build with that run. Nexus compares the run with the Wishlist assigned to that loadout. It saves only when overall Wishlist progress is higher, or equal with cleanup (fewer extra copies, or an even swap of requested copies). Not every run is saved.

Better means Wishlist progress, not Orb investment or keeping every individual Echo. Example: a run that loses 1 requested copy but gains 3 others is +2 overall. It can replace the Saved Build even when the lost Echo is one you value or obtained with Orbs. The save spends no Orbs, and it does not mean that Echo was judged worthless. Nexus cannot undo a completed save; Unassign does not restore a Saved Build. Keep Auto OFF if you want the current Saved Build left untouched. Auto starts OFF each session.
