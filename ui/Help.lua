...
5. Review recommendations with Auto OFF first. With Auto ON, Nexus may replace that active Saved Build with a better finished run; at level 80 after a finished run, that can happen right after you turn Auto ON or change the assignment. It can also happen right after you save edits to the assigned Wishlist. It treats the slot as a working copy, not a protected archive. Keep Auto OFF if you want it left unchanged.
...
Automatic save is separate from Take, Banish, Reroll and Freeze. After a completed run, Auto may replace your active Saved Build with that run and give it the Wishlist's name. Nexus compares the run with the Wishlist assigned to that loadout. It replaces an existing Saved Build only when overall Wishlist progress is higher than the Saved Build's, or equal with cleanup (some extra copies removed, or an even swap of requested copies). Not every run is saved. Assigning by itself changes nothing, but at level 80 after a finished run, turning Auto ON, changing the assigned Wishlist or saving edits to it makes Nexus check again right away, so a replacement can follow within seconds. There is no separate automatic-save switch: Auto OFF stops it, together with Nexus's other automatic Echo actions.
