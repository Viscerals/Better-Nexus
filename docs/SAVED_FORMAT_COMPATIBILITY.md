...
- **DPS records.** Read-only is not unreadable. The session's DPS store starts as a detached copy of the saved DPS records: the authority bundle's DPS data, or with no bundle the saved `dpsCapture`. The Leaderboard, DPS lookups and the Build Library's qualified view show them as before; nothing is written back, and received records are refused. An occupied bundle without DPS data shows none (legacy input is not revived). Saved DPS data that is not a plain table graph within the session copy bound (65,536 tables, 12 levels) is not shown, and the DPS debug log says so.
