...
| `Evidence.BeginRealDpsPairs` | Start bounded real-pair selection | resumable Leaderboard/tests | immutable Dummy/LK row sets -> private cursor | invalid row sets become empty inputs | W-UI |
| `Evidence.PumpRealDpsPairs` | Advance bounded real-pair selection | resumable Leaderboard/tests | cursor + positive work limit -> completion + work count | invalid cursor is complete; invalid limit performs one unit | W-UI |
| `Evidence.RealDpsPairsResult` | Read completed bounded pair result | resumable Leaderboard/tests | completed cursor -> one best pair per compatible identity; caller owns final sort | incomplete or invalid cursor returns unavailable | W-UI |
