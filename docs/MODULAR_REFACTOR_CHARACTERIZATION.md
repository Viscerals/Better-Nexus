...
| `Evidence.CurrentCopyAuthority` | Decide current locked-role mutation authority conjunctively | Community/Leaderboard/tests | current represented build + selected catalog provenance -> verified canonical owner verdict | unverified, relayed, peer-supplied, historical, or unknown provenance fails closed | W-C/W-UI |
...
| `Evidence.RealDpsPairs` | Select and rank deterministic real Dummy/LK pairs | Community/Leaderboard/retention/tests | immutable category rows -> strongest category representative per verified owner + ordinary + locked/full-combat identity, ranked by strongest single DPS | missing, conflicting, or unverified identity produces no pair; Average is presentation-only | W-C/W-UI |
| `Evidence.BeginRealDpsPairs` | Start bounded real-pair and shared-summary projection | resumable Leaderboard/tests | immutable Dummy/LK row sets -> private aggregation/sort cursor | invalid row sets become empty inputs | W-UI |
| `Evidence.PumpRealDpsPairs` | Advance bounded real-pair aggregation and merge sorting | resumable Leaderboard/tests | cursor + positive work limit -> completion + exact work count | invalid cursor is complete; invalid limit performs one unit | W-UI |
| `Evidence.RealDpsPairsResult` | Read completed bounded pair and summary projection | resumable Leaderboard/tests | completed cursor -> strongest-ranked compatible pairs + independent category-max summary | incomplete or invalid cursor returns unavailable | W-UI |
| `Evidence.DpsSummary` | Read the shared pair projection's independent category maxima | Community/sorting/tests | Dummy/LK row sets -> category maxima + strongest-ranked optional pair and presentation Average | Average remains unavailable when no real pair exists and never owns ranking | W-C/W-UI |
...
| `Sync.Housekeep` | Existing bounded expired-head cleanup without send or catalog work; defer terminal view refresh to ready update | Main lifecycle only when full Sync is gated | clock/queue -> expiry and terminal accounting | isolate at Main; preserve attributed errors | native housekeeping regression |
